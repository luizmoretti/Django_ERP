...
import { ArrowLeft, BoxIcon, BriefcaseBusiness, ChartBarBig, GitFork, HandCoins, Home, HousePlus, LogOut, Logs, Menu, Package, Package2, PackagePlus, PanelBottom, Settings2, User, Users } from "lucide-react";
import { Tooltip, TooltipProvider, TooltipTrigger, TooltipContent } from "../ui/tooltip";
import { useSidebar } from "./sidebarContext";


export function Sidebar() {
    const [StockOpen, setStockOpen] = useState(false); //State to manage Stock submenu visibility
    const {isSidebarVisible, setisSidebarVisible} = useSidebar()

    return (
        <div className="flex w-full flex-col bg-muted/40">
            <header className="fixed margin-top z-30 h-14 w-full bg-background border-b flex items-center px-4">
                <TooltipProvider>
                <Tooltip>
            ...
            <aside className={`fixed inset-y-0 left-0 z-10 w-64 border-r bg-background sm:flex flex-col transition-transform duration-300 ease-in-out
                ...
                <nav className="flex flex-col items-center gap-4 px-2 py-5">
                    ...
                    <div className={`${StockOpen ? "block" : "hidden"}pl-8`}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Link href="#" className="flex gap-3 px-4 py-2 shrink-0 items-center justify-center 
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Link href="/products" className="flex gap-3 px-4 py-2 shrink-0 items-center justify-center 
                                text-muted-foreground transition-colors hover:text-foreground whitespace-nowrap">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Link href="/categorie" className="flex gap-3 px-4 py-2 shrink-0 items-center justify-center 
                                text-muted-foreground transition-colors hover:text-foreground whitespace-nowrap">
            ...
            <div className="sm:hidden flex flex-col sm:gap-4 sm:py-4 sm:pl-14">
                <header className="sticky top-0 z-30 flex h-14 items-center px-4 border-b bg-background gap-14 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                    <Sheet>
                        ...
                        <SheetContent side="left" className="sm:max-w-x">
                            <nav className="grid gap-6 text-lg font-medium">
                                <Link href="#" className="flex h-10 w-10 rounded-full items-center justify-center text-primary-foreground md:text-base gap-2" prefetch={false}>
                                <img
                                src="/logo.png" alt="Logo" className="h-10 w-10 object-contain" />
                            <span className="sr-only">Logo</span>
                                ...
                                <Link href="#" className="flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" prefetch={false}>
                                    <ChartBarBig className="h-5 w-5 transition-all" />
                                    Dashboard
                                </Link>
                                <Link href="#" className="flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" prefetch={false}>
                                ...
                                <Link href="#" className="flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" prefetch={false}>
                                    <PackagePlus className="h-5 w-5 transition-all" />
                                    Categories
                                </Link>
                                <Link href="#" className="flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" prefetch={false}>
